...
func (r *Runner) executeStep(stepIdx, totalSteps int) error {
	...
	if r.options.DryRun {
		if r.stepRequiresWaves(tasks) {
			return r.dryRunWithWaves(tasks)
		}
		for _, t := range tasks {
	...
	if r.stepRequiresWaves(tasks) {
		return r.executeStepWithWaves(tasks)
	}
...
func (r *Runner) stepRequiresWaves(tasks []stepTask) bool {
	if r.options.NoDeps {
		return false
	}
	...
	for _, t := range tasks {
		if r.stepHasAnyTag(t.step, waveTags) {
			return true
		}
	}
...
func (r *Runner) executeStepWithWaves(tasks []stepTask) error {
	...
	for waveIdx, wave := range waves {
		r.output.PrintWaveHeader(waveIdx+1, len(waves))
		...
		for _, svc := range wave {
			...
			if failedDep != "" {
				r.output.PrintSkippedDependency(svc.DisplayName(), failedDep)
		...
		waveErr := r.executeTasksParallel(waveTasks)
		if waveErr != nil {
...
func (r *Runner) dryRunWithWaves(tasks []stepTask) error {
	services := make([]serviceinfo.ServiceInfo, len(tasks))
	for i, t := range tasks {
		services[i] = t.service
	}
