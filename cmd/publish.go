...
func PublishCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			if len(services) == 0 {
				fmt.Println("No services found to publish")
				return nil
			}
			...
			// Create and run the orchestrator - exclude the deploy step
			runner := orchestrator.NewRunner(services, recipes, orchestrator.RunnerOptions{
				...
				ExcludeSteps: []string{"deploy"}, // Exclude deploy-related steps
			})
